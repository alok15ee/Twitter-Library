...
def create_request(req_url):
    #try:
        ...
        #print 'From normal get request'
        #print r.headers['X-Rate-Limit-Remaining']
        #print r.status_code
        if (r.status_code==429):
            print 'true'
            time.sleep(10)
            create_request(req_url)
        else:
...
def create_request_pagination(req_url):
    ...
    #try:
        ...
        if (r.status_code==429):
            print 'true'
            time.sleep(10)
            create_request_pagination(req_url)
        else:
            jsonresponse = json.loads(r.text)
            return jsonresponse

    #except requests.ConnectionError:
        #print "create_request_pagination"
...
def create_request_campaign(req_url, camp_id):
    #try:
        ...
        if (r.status_code==429):
            print 'true'
            time.sleep(10)
            create_request_campaign_line_list(req_url, camp_id)
        else:
            #pass
            jsonresponse = json.loads(r.text)
            return jsonresponse
...
#    except requests.ConnectionError:
#        print "create_request_campaign_line_list"


def date_to_date_iso_format():
    current_time = datetime.utcnow()
    #print current_time.isoformat().split('.')[0] + 'Z'
    return current_time.isoformat().split('.')[0] + 'Z'
    ...
    #try:
        ...
        url_parameters = {
            ...
            'start_time' : '2016-01-01T13:20:42Z',
            'end_time' : date_to_date_iso_format(),
            'granularity' : 'TOTAL',
        ...
        if (r.status_code==429):
            print 'true'
            time.sleep(10)
            create_request_campaign_line_stat_list(req_url)
        else:
            jsonresponse = json.loads(r.text)
                #with open ('da2.txt', 'ab+') as f:
